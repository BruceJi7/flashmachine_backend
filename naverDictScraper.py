...
class WordIdiomWord():
    ...
    def __init__(self, searched_word, word_cell, definitions_cell):
    ...
    @property
    def language(self):
        english_present = DE_ENGLISH_RX.search(self.searched_word)
        if english_present:
            return 'English'
        else:
            return 'Korean'
...
def getDefinition(word):
    ...
    def word_idioms_to_objects(html_section):

        naver_word_results = html_section.find_all('dt')

        word_objects = []
        for w in naver_word_results:
            w_object = WordIdiomWord(word, w, w.find_next_sibling('dd'))
    ...
    def meanings_to_objects(html_section):
        word_titles = html_section.find_all('dt')
        word_objects = []

        for w in word_titles:
            w_object = MeaningsWord(word, w, w.find_next_sibling('dd'))
            w_dict_format = w_object.dictify
    ...
    if sections:
        ...
        word_idiom_section_word_objects = word_idioms_to_objects(sections[0])

    # If the word has a 'meanings' section - not all do
    if len(sections) > 1:
        #Sections[1] corresponds to 'meanings'
        meanings_section_word_objects =  meanings_to_objects(sections[1])
